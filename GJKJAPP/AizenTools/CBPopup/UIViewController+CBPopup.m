...
@property (assign, nonatomic) CGRect beforeFrame;
...
-(void)setBeforeFrame:(CGRect)beforeFrame {
    ...
    [self willChangeValueForKey:@"beforeFrame"];

    objc_setAssociatedObject(self,
                             @selector(beforeFrame),
                             [NSValue valueWithCGRect:beforeFrame],
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [self didChangeValueForKey:@"beforeFrame"];

}
-(CGRect)beforeFrame {
    NSValue *rectValue = objc_getAssociatedObject(self, @selector(beforeFrame));
...
- (void)cb_updateViewHierachy
{
    if(!self.cb_overlayView.superview) {
        ...
        if (!self.cb_maskView) {
            ...
            self.cb_maskView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];//[UIColor colorWithWhite:0 alpha:0.5];
...
- (void)cb_settingHiiden:(BOOL)isHiiden {
    
    self.view.superview.hidden = isHiiden;
    ...
    if (isHiiden) {
        ...
        frame.origin.y = self.beforeFrame.origin.y;
    }
