...
-(void)br_configSubView {
    return;
    if (self.isNeedSound == NO) {
        [_voiceButton removeFromSuperview];
        _voiceButton = nil;
    }
    if (self.isNeedFace == NO) {
...
-(void)setFrame:(CGRect)frame
{
    ...
    float y = self.frameHeight - 37 - (HEIGHT_TABBAR - 37) / 2;
...
{
    if (_faceButton == nil) {
        CGFloat CHATBOX_BUTTON_WIDTH_1 = 37.0;
        _faceButton = [[UIButton alloc] initWithFrame:CGRectMake(self.moreButton.originX - CHATBOX_BUTTON_WIDTH_1, (HEIGHT_TABBAR - CHATBOX_BUTTON_WIDTH_1) / 2, CHATBOX_BUTTON_WIDTH_1, CHATBOX_BUTTON_WIDTH_1)];
        [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotion"] forState:UIControlStateNormal];
        [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotionHL"] forState:UIControlStateHighlighted];
