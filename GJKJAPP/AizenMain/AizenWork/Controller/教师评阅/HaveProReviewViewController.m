...
@interface HaveProReviewViewController ()
{
    int _page;
}
...
@implementation HaveProReviewViewController
...
-(NSArray *) detailLayout:(NSArray *)dataArr{
    ...
    for (int i = 0 ; i < dataArr.count; i++) {
        ...
        model.project = @"";
        if (![[dict objectForKey:@"ProjectName"]isKindOfClass:[NSNull class]]) {
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ProReviewModel *model = self.tableArray[indexPath.row];
    if ([model.proID isEqualToString:@"0"]) {
        [self.view makeToast:@"该学生暂时不能被评阅!" duration:2.0 position:@"center"];
        return;
    }
    LWReadViewController *vc = getControllerFromStoryBoard(@"Worker", @"lwreadSBID");
    vc.lzId = model.proID;
