...
@interface HCCheckReportViewController ()
{
    int _page;
}
...
@implementation HCCheckReportViewController
...
-(void)getDataSourceFromHttpIsFooterRefresh:(BOOL)FooterRefresh
{
    ...
    [HttpService GetOverCheckTeamRecordList:batchID ActivityID:activity Rows:20 and:1 success:^(id  _Nonnull responseObject) {
        if (FooterRefresh == YES) {
...
-(void)startLayout
{
    ...
    [passBtn setTitle:@"撤销审核" forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.zhuangtaiLb.text = [NSString stringWithFormat:@"状态:%@",model.CheckState==1?@"审核通过":@"审核不通过"];
...
-(void)check
{
    ...
    vc.pid = [string copy];
    vc.shenheType = shenheType_reportcexiao;
