//
//  SignsuccessDetailViewController.m
//  GJKJAPP
//
...
#import "SignsuccessDetailViewController.h"
...
@interface SignsuccessDetailViewController ()<MKMapViewDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,CLLocationManagerDelegate,MKMapViewDelegate>
...
- (void)viewDidLoad {
    ...
    NSString *title = [NSString stringWithFormat:@"%@的签到审核",[_getArr objectForKey:@"UserName"]];
...
-(void) setAnnotation:(NSArray *)dataArr{
    if (![dataArr isKindOfClass:[NSArray class]] || dataArr.count <= 0) {
        [BaseViewController br_showAlterMsg:@"暂无数据"];
        return;
    }
    ...
    CLLocationDegrees lat = [[getDic objectForKey:@"CheckInLat"] doubleValue];
    CLLocationDegrees lon = [[getDic objectForKey:@"CheckInLon"] doubleValue];
    annotation.coordinate = CLLocationCoordinate2DMake(lat,lon);
    annotation.title = @"打卡地点";
    annotation.subtitle = [getDic objectForKey:@"CheckInPlace"];
    ...
    _auditValue.text = @"上班签到";//[[_getArr objectForKey:@"ID"] stringValue];
    ...
    NSRange rang = {0,10};
    NSString *timeStartStr = [[[[_getArr objectForKey:@"CheckInDate"] stringByReplacingOccurrencesOfString:@"/Date(" withString:@""] stringByReplacingOccurrencesOfString:@")/" withString:@""]substringWithRange:rang];
