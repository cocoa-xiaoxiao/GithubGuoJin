...
#import "People.h"
#import "PhoneInfo.h"
@interface NotProListViewController ()
{
    int _page;
}
...
@implementation NotProListViewController
...
-(void)getDataSourceFromHttpIsFooterRefresh:(BOOL)FooterRefresh
{
    ...
    [HttpService GetMyProjectApplyCheckList:CurrAdminID and:batchID and:@"" and:@"" and:10 and:_page success:^(NSDictionary * responseObject) {
        if (FooterRefresh == YES) {
...
-(NSArray *) detailLayout:(NSArray *)dataArr{
    ...
    for (int i = 0 ; i < dataArr.count; i++) {
        ...
        if ([[dict objectForKey:@"IsTeam"] boolValue]) {
            model.pteam = @"是";
        }else{
            model.pteam = @"否";
        }
