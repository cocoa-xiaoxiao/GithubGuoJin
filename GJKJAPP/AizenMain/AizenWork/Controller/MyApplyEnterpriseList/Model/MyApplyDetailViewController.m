...
@interface MyApplyDetailViewController (){
    int *number;
...
@implementation MyApplyDetailViewController
...
-(void) startLayout{
    ...
    NSString *titleStr = [NSString stringWithFormat:@"%@",[dataDic objectForKey:@"PositionName"]];
    ...
    stationLab.text = [NSString stringWithFormat:@"到岗日期:%@",[PhoneInfo timestampSwitchTime:[comeTime integerValue] andFormatter:@"YYYY-MM-dd"]];
...
//        cancelBtn.frame = CGRectMake(detailView.frame.size.width * 0.75, titleLab.frame.origin.y, detailView.frame.size.width * 0.2, detailView.frame.size.height * 0.18);
//        cancelBtn.layer.cornerRadius = 5;
//        cancelBtn.layer.masksToBounds = YES;
//        [cancelBtn setTitle:@"撤销" forState:UIControlStateNormal];
//        cancelBtn.backgroundColor = [UIColor colorWithRed:225/255.0 green:60/255.0 blue:33/255.0 alpha:1];
//        [cancelBtn addTarget:self action:@selector(cancelAction:) forControlEvents:UIControlEventTouchUpInside];
//        cancelBtn.accessibilityElements = dataDic;
...
-(void) cancelAction:(UIButton *)sender{
    NSString *ID = [[(id)sender.accessibilityElements objectForKey:@"ID"] stringValue];
    NSString *currTime = [PhoneInfo getNowTimeTimestamp3];
    
    NSString *tokenStr = [NSString stringWithFormat:@"%@GJApply%@",ID,currTime];
    NSString *encodeToken = [AizenMD5 MD5ForUpper16Bate:tokenStr];
    
    NSString *url = [NSString stringWithFormat:@"%@/ApiInternshipApplyEnterpriseInfo/Delete?ApplyID=%@&TimeStamp=%@&Token=%@",kCacheHttpRoot,ID,currTime,encodeToken];
    NSLog(@"%@",url);
