...
@interface ModifyStationDetailViewController ()<UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating, UISearchBarDelegate>
...
@implementation ModifyStationDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"岗位输入";
    ...
    _searchController.searchBar.placeholder = @"请输入岗位名称";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_searchController isActive]) {
        cell.textLabel.text = [[_results objectAtIndex:indexPath.row] objectForKey:@"PositionName"];
    } else {
        cell.textLabel.text = [[_datas objectAtIndex:indexPath.row] objectForKey:@"PositionName"];
    }
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    NSString *url = [NSString stringWithFormat:@"%@/ApiEnterpriseInfo/getpositionlist?EnterpriseID=%@&PositionName=%@",kCacheHttpRoot,_uploadID,inputStr];
    NSLog(@"%@",url);
...
-(void) handleData:(NSArray *)dataArr{
    ...
    for(int i = 0;i<[dataArr count];i++){
        ...
        [setDic setObject:[[dataArr objectAtIndex:i] objectForKey:@"PositionName"] forKey:@"PositionName"];
        [setDic setObject:[[[dataArr objectAtIndex:i] objectForKey:@"ID"] stringValue] forKey:@"ID"];
        [setDic setObject:@"station" forKey:@"Type"];
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    NSDictionary *setDic = [[NSDictionary alloc]initWithObjectsAndKeys:inputStr,@"PositionName",@"0",@"ID",@"station",@"Type",nil];
    _searchController.active = NO;
