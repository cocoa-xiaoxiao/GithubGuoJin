//
//  InspectionStactisticViewController.m
//  GJKJAPP
//
...
#import "InspectionStactisticViewController.h"
...
#import "AizenHttp.h"
@interface InspectionStactisticViewController ()<UITableViewDelegate,UITableViewDataSource,CZPickerViewDelegate,CZPickerViewDataSource>
{
    NSString *_selebumen;
}
...
@implementation InspectionStactisticViewController
...
-(void)httpFieldWithDeptID:(NSString *)ID
{
    ...
    [HttpService GetStatisticsListDeptID:ID ActivityID:batchID success:^(id  _Nonnull responseObject) {
        NSArray *array = [responseObject objectForKey:@"AppendData"][@"rows"];
        for (int i = 0; i < array.count; i++) {
            NSDictionary *dict = array[i];
            UnitModel *model = [[UnitModel alloc]init];
            model.renshu = [dict[@"jczs"] intValue];
            model.name = dict[@"DepartmentName"];
            model.unit_zg = [dict[@"yjrs"] intValue];
            model.unit_dg = model.renshu - model.unit_zg;
            model.unit_sxl = 0;
            if (model.unit_zg > 0) {
                model.unit_sxl = model.unit_zg/(model.renshu*1.0);
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.name3.attributedText = [NSAttributedString xo_changeFontWithFont:[UIFont systemFontOfSize:13] totalStr:[NSString stringWithFormat:@"已检查：%d人",model.unit_zg] andChangeStingArray:@[@"已检查："]andColor:[UIColor darkGrayColor]];
    cell.name4.attributedText = [NSAttributedString xo_changeFontWithFont:[UIFont systemFontOfSize:13] totalStr:[NSString stringWithFormat:@"待检查：%d人",model.unit_dg] andChangeStingArray:@[@"待检查："]andColor:[UIColor darkGrayColor]];
    cell.degress = model.unit_sxl;
    cell.shixilvLabel.text = [NSString stringWithFormat:@"%.2f%%\n通过率",model.unit_sxl*100];
    return cell;
}
